...
public class ResourceAllocationAction
    ...
    protected void allocate(FlowMirrorPointCreateFsm stateMachine)
        ...
        transactionManager.doInTransaction(() -> {
            ...
            FlowMirrorPoints flowMirrorPoints = foundFlowMirrorPoints.isPresent()
                    ? foundFlowMirrorPoints.get() : createFlowMirrorPoints(mirrorPoint, flowPath);

            Switch destSwitch = switchRepository.findById(mirrorPoint.getReceiverPointSwitchId())
                    .orElseThrow(() -> new FlowProcessingException(ErrorType.NOT_FOUND,
                            format("Switch %s not found", mirrorPoint.getReceiverPointSwitchId())));

            stateMachine.setMirrorPathId(new PathId(mirrorPoint.getMirrorPointId()));
            ...
            FlowSegmentCookieBuilder cookieBuilder = FlowSegmentCookie.builder()
                    .flowEffectiveId(stateMachine.getUnmaskedCookie());

            SwitchProperties switchProperties = getSwitchProperties(destSwitch.getSwitchId());
            ...
            FlowMirrorPath flowMirrorPath = FlowMirrorPath.builder()
                    ...
                    .srcSwitch(flowMirrorPoints.getMirrorSwitch())
                    .destSwitch(destSwitch)
                    .destPort(mirrorPoint.getReceiverPointPort())
                    .destVlan(mirrorPoint.getReceiverPointVlan())
                    .cookie(cookieBuilder.direction(
                            flowPath.getCookie().getDirection()).type(CookieType.MIRROR_FLOW).build())
