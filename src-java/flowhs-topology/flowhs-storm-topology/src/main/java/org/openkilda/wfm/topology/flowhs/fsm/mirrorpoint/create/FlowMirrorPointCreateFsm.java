...
import org.openkilda.wfm.topology.flowhs.fsm.mirrorpoint.create.actions.EmitReInstallRulesRequestsAction;
...
import org.openkilda.wfm.topology.flowhs.fsm.mirrorpoint.create.actions.OnReceivedReInstallResponseAction;
...
public final class FlowMirrorPointCreateFsm
    ...
    private boolean rulesReInstalled = false;

    private final Map<UUID, FlowSegmentRequestFactory> reInstallCommands = new HashMap<>();
    ...
    public static class Factory {
        ...
        public Factory(FlowMirrorPointCreateHubCarrier carrier, PersistenceManager persistenceManager,
            ...
            builder.transition().from(State.RESOURCE_ALLOCATION_COMPLETED).to(State.REINSTALLING_RULES)
                    .on(Event.NEXT)
                    .perform(new EmitReInstallRulesRequestsAction(persistenceManager, resourcesManager));
            builder.transitions().from(State.RESOURCE_ALLOCATION_COMPLETED)
                    .toAmong(State.FLOW_MIRROR_PATH_ALLOCATION_REVERTED, State.FLOW_MIRROR_PATH_ALLOCATION_REVERTED)
                    .onEach(Event.TIMEOUT, Event.ERROR);

            builder.internalTransition().within(State.REINSTALLING_RULES).on(Event.RESPONSE_RECEIVED)
                    .perform(new OnReceivedReInstallResponseAction(speakerCommandRetriesLimit));
            builder.internalTransition().within(State.REINSTALLING_RULES).on(Event.ERROR_RECEIVED)
                    .perform(new OnReceivedReInstallResponseAction(speakerCommandRetriesLimit));
            builder.transition().from(State.REINSTALLING_RULES).to(State.VALIDATING_RULES)
                    .on(Event.RULES_REINSTALLED)
                    .perform(new EmitVerifyRulesRequestsAction());
            builder.transitions().from(State.REINSTALLING_RULES)
            ...
            builder.transition().from(State.FLOW_MIRROR_PATH_ALLOCATION_REVERTED).to(State.REINSTALLING_NEW_RULES)
                    .on(Event.NEXT);
            builder.transition().from(State.FLOW_MIRROR_PATH_ALLOCATION_REVERTED)
                    .to(State.FLOW_MIRROR_POINTS_RECORD_PROCESSED)
                    .on(Event.SKIP_REINSTALLING_RULES);
            ...
            builder.onEntry(State.REINSTALLING_NEW_RULES)
                    .perform(new EmitReInstallRulesRequestsAction(persistenceManager, resourcesManager));
            builder.internalTransition().within(State.REINSTALLING_NEW_RULES).on(Event.RESPONSE_RECEIVED)
                    .perform(new OnReceivedReInstallResponseAction(speakerCommandRetriesLimit));
            builder.internalTransition().within(State.REINSTALLING_NEW_RULES).on(Event.ERROR_RECEIVED)
                    .perform(new OnReceivedReInstallResponseAction(speakerCommandRetriesLimit));
            builder.transition().from(State.REINSTALLING_NEW_RULES).to(State.NEW_RULES_REINSTALLED)
                    .on(Event.RULES_REMOVED);
            builder.transition().from(State.REINSTALLING_NEW_RULES).to(State.NEW_RULES_REINSTALLED)
                    .on(Event.ERROR)
                    .perform(new HandleNotCompletedCommandsAction());

            builder.transitions().from(State.NEW_RULES_REINSTALLED)
    ...
    public enum State {
        ...
        REINSTALLING_NEW_RULES,
        NEW_RULES_REINSTALLED,
