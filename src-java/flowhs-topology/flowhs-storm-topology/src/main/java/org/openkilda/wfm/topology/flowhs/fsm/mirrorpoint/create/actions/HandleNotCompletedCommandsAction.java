...
public class HandleNotCompletedCommandsAction
    ...
    public void perform(State from, State to, Event event, FlowMirrorPointCreateContext context,
        ...
        for (UUID commandId : stateMachine.getPendingCommands().keySet()) {
            FlowSegmentRequestFactory reInstallCommand = stateMachine.getReInstallCommands().get(commandId);
            if (reInstallCommand != null) {
                stateMachine.saveErrorToHistory("Command is not finished yet",
                        format("Completing the re-install operation although the remove command may not be "
                                        + "finished yet: commandId %s, switch %s, cookie %s", commandId,
                                reInstallCommand.getSwitchId(), reInstallCommand.getCookie()));
            }
        }
