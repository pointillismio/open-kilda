...
import org.openkilda.wfm.topology.flowhs.fsm.mirrorpoint.delete.actions.DeallocateFlowMirrorPathResoursesAction;
import org.openkilda.wfm.topology.flowhs.fsm.mirrorpoint.delete.actions.EmitReInstallRulesRequestsAction;
...
import org.openkilda.wfm.topology.flowhs.fsm.mirrorpoint.delete.actions.OnReceivedReInstallResponseAction;
    ...
    public static class Factory {
        ...
        public Factory(FlowMirrorPointDeleteHubCarrier carrier, PersistenceManager persistenceManager,
            ...
            builder.onEntry(State.DEALLOCATING_FLOW_MIRROR_PATH_RESOURCES)
                    .perform(new DeallocateFlowMirrorPathResoursesAction(persistenceManager, resourcesManager));
            ...
            builder.transition().from(State.FLOW_MIRROR_PATH_RESOURCES_DEALLOCATED).to(State.REINSTALLING_RULES)
                    .on(Event.NEXT);
            ...
            builder.onEntry(State.REINSTALLING_RULES)
            ...
            builder.internalTransition().within(State.REINSTALLING_RULES).on(Event.ERROR_RECEIVED)
                    .perform(new OnReceivedReInstallResponseAction(speakerCommandRetriesLimit));
            builder.transition().from(State.REINSTALLING_RULES).to(State.RULES_REINSTALLED)
                    .on(Event.RULES_REMOVED);
            builder.transition().from(State.REINSTALLING_RULES).to(State.RULES_REINSTALLED)
                    .on(Event.ERROR)
                    .perform(new HandleNotCompletedCommandsAction());

            builder.transitions().from(State.RULES_REINSTALLED)
